...
#hpp, labels = helpers.readFromCSV("csv/hpp_train_new.csv")
...
# #hpp_test, labels_test = helpers.readFromCSV("csv/hpp_test_new.csv")
...
# #hpp_test_new, labels_test_new = helpers.readFromCSV(
# #    "csv/hpp_test_images_new.csv")
...
allFeatures = np.concatenate(tuple([lpq,toe, tos, wor,	hvsl, lvl,hpp]), axis=1)
# allFeaturesTest = np.concatenate(
#     tuple([toe_test, tos_test, wor_test,	hvsl_test, lvl_test]), axis=1)
# allFeaturesTestNew = np.concatenate(tuple(
#     [toe_test_new, tos_test_new, wor_test_new,	hvsl_test_new, lvl_test_new]), axis=1)
